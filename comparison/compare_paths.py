...
def get_path_labels(components: List[str]) -> Tuple[str, List[str]]:
    """
    ...
    :return: a tuple of the contribution label, and list of labels skipping the first one which is the contribution
    """
    label_list = [neo4j.get_resource_label(components[i]).strip().lower() if i % 2 == 0
...
def compare_resources(resources: List[str]):
    ...
    for _, paths in paths_dict.items():
        for i in range(len(paths)):
            paths_set.add(stringify(paths[i]['path_labels'][1]))
    ...
    for idx, cont_id in enumerate(resources):
        for path in paths_dict[cont_id]:
            paths_contribution_matrix[paths_indices[stringify(path['path_labels'][1])], idx] = True
    ...
    for path, path_idx in paths_indices.items():
        values = []
        for idx, cont_id in enumerate(resources):
            candidates_values = list(filter(lambda p: stringify(p['path_labels'][1]) == path, paths_dict[cont_id]))
            if len(candidates_values) == 0:
                ...
                for value in candidates_values:
                    cont_pred_values.append({
                        ...
                        "pathLabels": [value['path_labels'][0]] + value['path_labels'][1],
