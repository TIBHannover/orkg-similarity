...
orkg = ORKG(host=os.environ["API_HOST"], simcomp_host=os.environ["SIMCOMP_HOST"])
...
class ReviewAPI(MethodView):
    ...
    def get(self, reqargs):
        ...
        review_data = VisualizationResponse.get_by_resource_id(resource_id)
        if not review_data:
            abort(404)
        ...
        for section_id in reversed(sections):
            ...
            if "ComparisonSection" in section_resource['classes']:
                ...
                comparison_table = orkg.contributions.compare_dataframe(comparison_id=comparison_id, like_ui=True).to_html()
